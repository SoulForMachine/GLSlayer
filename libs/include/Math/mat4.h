...
public:
	...
	static const int el_count = 16;


	mat4() {}
	mat4(_ST m0, _ST m1, _ST m2, _ST m3, 
		 _ST m4, _ST m5, _ST m6, _ST m7,
		 _ST m8, _ST m9, _ST m10, _ST m11,
		 _ST m12, _ST m13, _ST m14, _ST m15
		 );
	mat4(const vec4<_ST>& x, const vec4<_ST>& y, const vec4<_ST>& z, const vec4<_ST>& w);

// conversion constructors
	explicit mat4(const mat3<_ST>& m3);
	explicit mat4(const _ST* m);
	template <class _T>
		explicit mat4(const mat4<_T>& m)
	{
		set(_ST(m(0)), _ST(m(1)), _ST(m(2)), _ST(m(3)), 
...
// access
	...
	vec4<_ST>& operator [] (int i);
	const vec4<_ST>& operator [] (int i) const;
	_ST& operator () (int i);
	...
	void set(_ST m0, _ST m1, _ST m2, _ST m3,
			 ...
			 _ST m8, _ST m9, _ST m10, _ST m11,
			 _ST m12, _ST m13, _ST m14, _ST m15);
	void set(const vec4<_ST>& x, const vec4<_ST>& y, const vec4<_ST>& z, const vec4<_ST>& w);
	void set(const mat3<_ST>& m3);
	// set only 3x3 submatrix
	void set3x3(const mat3<_ST>& m3);
	void get(vec4<_ST>& x, vec4<_ST>& y, vec4<_ST>& z, vec4<_ST>& w) const;
	void get(mat3<_ST>& m3) const;
	vec4<_ST> get_col(int col) const;
...
template <class _ST>
const mat4<_ST> mat4<_ST>::identity
(
...
template <class _ST>
inline
mat4<_ST>::mat4(const vec4<_ST>& _x, const vec4<_ST>& _y, const vec4<_ST>& _z, const vec4<_ST>& _w)
{
	set(_x, _y, _z, _w);
}
...
template <class _ST>
inline
vec4<_ST>& mat4<_ST>::operator [] (int i)
{
	return *(vec4<_ST>*) (_data + (i << 2));
}

template <class _ST>
inline
const vec4<_ST>& mat4<_ST>::operator [] (int i) const
{
	return *(vec4<_ST>*) (_data + (i << 2));
}
...
template <class _ST>
inline
_ST mat4<_ST>::operator () (int i) const
{
	return _data[i];
}
...
template <class _ST>
inline
_ST& mat4<_ST>::operator () (int row, int col)
{
	return _data[(row << 2) + col];
}

template <class _ST>
inline
_ST mat4<_ST>::operator () (int row, int col) const
{
	return _data[(row << 2) + col];
}
...
template <class _ST>
void mat4<_ST>::set(const vec4<_ST>& _x, const vec4<_ST>& _y, const vec4<_ST>& _z, const vec4<_ST>& _w)
{
	row0 = _x;
	row1 = _y;
	row2 = _z;
...
{
	row0.set(m3.row0, _ST(0));
	row1.set(m3.row1, _ST(0));
	row2.set(m3.row2, _ST(0));
	row3.set(_ST(0), _ST(0), _ST(0), _ST(1));
}
...
template <class _ST>
void mat4<_ST>::get(vec4<_ST>& _x, vec4<_ST>& _y, vec4<_ST>& _z, vec4<_ST>& _w) const
{
	_x = row0;
	_y = row1;
	_z = row2;
...
{
	return vec4<_ST>(_data[col], _data[col + 4], _data[col + 8], _data[col + 12]);
}
...
{
	_swap(_data[1], _data[4]);
	_swap(_data[2], _data[8]);
	_swap(_data[3], _data[12]);
	_swap(_data[6], _data[9]);
	_swap(_data[7], _data[13]);
	_swap(_data[11], _data[14]);
}
...
{
	row0.set(_ST(1), _ST(0), _ST(0), _ST(0));
	row1.set(_ST(0), _ST(1), _ST(0), _ST(0));
	row2.set(_ST(0), _ST(0), _ST(1), _ST(0));
	row3.set(transl);
}
...
void mat4<_ST>::set_rotation_x(_ST angle)
{
	...
	row0.set(_ST(1), _ST(0), _ST(0), _ST(0));
	row1.set(_ST(0), cos_a, sin_a, _ST(0));
	row2.set(_ST(0), -sin_a, cos_a, _ST(0));
	row3.set(_ST(0), _ST(0), _ST(0), _ST(1));
}
...
void mat4<_ST>::set_rotation_y(_ST angle)
{
	...
	row0.set(cos_a, _ST(0), -sin_a, _ST(0));
	row1.set(_ST(0), _ST(1), _ST(0), _ST(0));
	row2.set(sin_a, _ST(0), cos_a, _ST(0));
	row3.set(_ST(0), _ST(0), _ST(0), _ST(1));
}
...
void mat4<_ST>::set_rotation_z(_ST angle)
{
	...
	row0.set(cos_a, sin_a, _ST(0), _ST(0));
	row1.set(-sin_a, cos_a, _ST(0), _ST(0));
	row2.set(_ST(0), _ST(0), _ST(1), _ST(0));
	row3.set(_ST(0), _ST(0), _ST(0), _ST(1));
}
...
{
	row0.set(s, _ST(0), _ST(0), _ST(0));
	row1.set(_ST(0), s, _ST(0), _ST(0));
	row2.set(_ST(0), _ST(0), s, _ST(0));
	row3.set(_ST(0), _ST(0), _ST(0), _ST(1));
}
...
{
	row0.set(_x, _ST(0), _ST(0), _ST(0));
	row1.set(_ST(0), _y, _ST(0), _ST(0));
	row2.set(_ST(0), _ST(0), _z, _ST(0));
	row3.set(_ST(0), _ST(0), _ST(0), _ST(1));
}
...
{
	row0.set(s.x, _ST(0), _ST(0), _ST(0));
	row1.set(_ST(0), s.y, _ST(0), _ST(0));
	row2.set(_ST(0), _ST(0), s.z, _ST(0));
	row3.set(_ST(0), _ST(0), _ST(0), _ST(1));
}
...
void mat4<_ST>::ortho(_ST left, _ST right, _ST bottom, _ST top, _ST znear, _ST zfar)
{
	...
	row0.set(_ST(2) * inv_width, _ST(0), _ST(0), _ST(0));
	row1.set(_ST(0), _ST(2) * inv_height, _ST(0), _ST(0));
	row2.set(_ST(0), _ST(0), - _ST(2) * inv_depth, _ST(0));
	row3.set(-(right + left) * inv_width, -(top + bottom) * inv_height, -(zfar + znear) * inv_depth, _ST(1));
}
...
void mat4<_ST>::frustum(_ST left, _ST right, _ST bottom, _ST top, _ST znear, _ST zfar)
{
	...
	row0.set(two_near * inv_width, _ST(0), _ST(0), _ST(0));
	row1.set(_ST(0), two_near * inv_height, _ST(0), _ST(0));
	row2.set((right + left) * inv_width, (top + bottom) * inv_height, -(zfar + znear) * inv_depth, _ST(-1));
	row3.set(_ST(0), _ST(0), - two_near * zfar * inv_depth, _ST(0));
}
...
void mat4<_ST>::frustum_inf(_ST left, _ST right, _ST bottom, _ST top, _ST znear)
{
	...
	row0.set(two_near * inv_width, _ST(0), _ST(0), _ST(0));
	row1.set(_ST(0), two_near * inv_height, _ST(0), _ST(0));
	row2.set((right + left) * inv_width, (top + bottom) * inv_height, _ST(-1), _ST(-1));
	row3.set(_ST(0), _ST(0), -two_near, _ST(0));
}
...
{
	fovy *= _ST(0.5);
	_ST y_scale = cos(fovy) / sin(fovy);
	_ST x_scale = y_scale / aspect;
	_ST inv_delta = _ST(1.0) / (zfar - znear);
	_ST z_scale = -(znear + zfar) * inv_delta;

	row0.set(x_scale, _ST(0), _ST(0), _ST(0));
	row1.set(_ST(0), y_scale, _ST(0), _ST(0));
	row2.set(_ST(0), _ST(0), z_scale, _ST(-1));
	row3.set(_ST(0), _ST(0), _ST(-2.0) * znear * zfar * inv_delta, _ST(0));
}
...
{
	memcpy(_data, src, sizeof(_ST) * el_count);
}
...
template <class _ST>
inline
void mat4<_ST>::_swap(_ST& a, _ST& b)
{
	_ST t = a;
	a = b;
	b = t;
